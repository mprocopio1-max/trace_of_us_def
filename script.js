import * as THREE from "three";
      ...
      class WebcamPresenceTracker {
        constructor() {
          ...
          // Secondary hand-motion detector: used only when MediaPipe misses the hand.
          // It ignores over-bright plasma flicker and looks for larger, coherent, darker moving shapes.
          ...
          this.motionFallbackCenter = new THREE.Vector2(0.5, 0.5);
          this.motionFallbackLastCenter = null;
          this.motionFallbackHold = 0;
        ...
        loop() {
          ...
          Promise.all([
            ...
            let handData = this.extractHandData(hands, deltaSeconds);
            const motionHandData = this.extractMotionHandFallback(deltaSeconds);

            // Use a filtered motion fallback only when the hand model is not seeing the hand.
            // This keeps plasma-sphere light from becoming a trace, but helps when the hand is partially hidden.
            if (!handData.present && motionHandData.present) {
              handData = motionHandData;
            }

            const presenceDetected = handData.present;

            if (presenceDetected) {
              const combinedCenter = new THREE.Vector2();
              const combinedWeight = [];

              // The trace follows the hand. The face is used only as a very light spatial stabilizer.
              if (faceData.present) {
                combinedCenter.addScaledVector(faceData.center, 0.18);
                combinedWeight.push(0.18);
              }

              if (handData.present) {
                combinedCenter.addScaledVector(handData.center, 0.82);
                combinedWeight.push(0.82);
              }
              ...
              this.state = {
                ...
                confidence: clamp01(handData.confidence * 0.85 + faceData.confidence * 0.15),
              };
        ...
        extractMotionHandFallback(deltaSeconds) {
          const width = this.motionCanvas.width;
          const height = this.motionCanvas.height;

          if (!this.motionCtx || !this.video.videoWidth || !this.video.videoHeight) {
            return this.emptyHandData();
          }

          try {
            this.motionCtx.drawImage(this.video, 0, 0, width, height);
          } catch (error) {
            return this.emptyHandData();
          }

          const frame = this.motionCtx.getImageData(0, 0, width, height).data;
          if (!this.previousMotionFrame) {
            this.previousMotionFrame = new Uint8ClampedArray(frame);
            return this.emptyHandData();
          }
          ...
          let maxY = 0;
          let activePixels = 0;
          let brightRejected = 0;

          // Sample every other pixel for stability and speed.
          for (let y = 2; y < height - 2; y += 2) {
            for (let x = 2; x < width - 2; x += 2) {
              ...
              const g = frame[i + 1];
              const b = frame[i + 2];
              const pr = this.previousMotionFrame[i];
              const pg = this.previousMotionFrame[i + 1];
              const pb = this.previousMotionFrame[i + 2];

              const brightness = (r + g + b) / 3;
              const previousBrightness = (pr + pg + pb) / 3;
              const maxChannel = Math.max(r, g, b);
              const minChannel = Math.min(r, g, b);
              const saturation = maxChannel - minChannel;
              const diff = Math.abs(brightness - previousBrightness) + Math.abs(r - pr) * 0.25 + Math.abs(g - pg) * 0.25 + Math.abs(b - pb) * 0.25;

              // Plasma cores are usually very bright, saturated, and flickery: reject them.
              if (brightness > 205 || previousBrightness > 210 || (brightness > 165 && saturation > 85)) {
                if (diff > 22) brightRejected++;
                continue;
              }

              // Hands generally create broader medium/dark motion rather than tiny electric flicker.
              if (diff > 18 && brightness > 18 && brightness < 185) {
                const weight = Math.min(1, diff / 80) * (1 - Math.max(0, brightness - 150) / 80);
                sumX += x * weight;
                sumY += y * weight;
                weightSum += weight;
                activePixels++;
          ...
          // Slowly update previous frame to remove static background while keeping hand motion.
          for (let i = 0; i < frame.length; i += 4) {
            this.previousMotionFrame[i] = this.previousMotionFrame[i] * 0.72 + frame[i] * 0.28;
            this.previousMotionFrame[i + 1] = this.previousMotionFrame[i + 1] * 0.72 + frame[i + 1] * 0.28;
            this.previousMotionFrame[i + 2] = this.previousMotionFrame[i + 2] * 0.72 + frame[i + 2] * 0.28;
            this.previousMotionFrame[i + 3] = 255;
          }

          const bboxW = Math.max(0, maxX - minX);
          const bboxH = Math.max(0, maxY - minY);
          const bboxArea = bboxW * bboxH;
          const coherentEnough = activePixels > 18 && bboxArea > 90 && bboxArea < width * height * 0.55;
          const notOnlyPlasmaFlicker = activePixels > brightRejected * 0.22;

          if (!coherentEnough || !notOnlyPlasmaFlicker || weightSum <= 0) {
            this.motionFallbackHold = Math.max(0, this.motionFallbackHold - deltaSeconds * 2.6);
            if (this.motionFallbackHold <= 0) {
              this.motionFallbackLastCenter = null;
              return this.emptyHandData();
            }
          }
          ...
          if (coherentEnough && notOnlyPlasmaFlicker && weightSum > 0) {
            const rawCenter = new THREE.Vector2(sumX / weightSum / width, sumY / weightSum / height);
            this.motionFallbackCenter.lerp(rawCenter, this.motionFallbackLastCenter ? 0.35 : 1);
            this.motionFallbackHold = Math.min(0.55, this.motionFallbackHold + deltaSeconds * 4.0);
          }

          if (this.motionFallbackHold <= 0.10) {
            return this.emptyHandData();
          }

          const center = this.motionFallbackCenter.clone();
          const movement = this.motionFallbackLastCenter ? center.distanceTo(this.motionFallbackLastCenter) : 0.018;
          this.motionFallbackLastCenter = center.clone();
          const spread = clamp01(((bboxW / width) + (bboxH / height)) * 0.9);
          const center3D = new THREE.Vector3(
            THREE.MathUtils.lerp(-165, 165, center.x),
            THREE.MathUtils.lerp(115, -85, center.y),
            THREE.MathUtils.lerp(110, -110, spread)
          );
          ...
          return {
            ...
            center,
            center3D,
            leftCenter3D: center3D.clone(),
            rightCenter3D: new THREE.Vector3(0, 0, 0),
            movement: Math.max(0.012, movement + activePixels / 9000),
            handMovement: Math.max(0.012, movement),
            handSpread: Math.max(0.12, spread),
            gestureEnergy: clamp01(movement * 8 + spread * 0.25),
            twoHandBridge: 0,
            faceHandProximity: 0,
            gestureType: "gesture",
            handOpen: 0.65,
            handClose: 0.2,
            handCount: 1,
            confidence: clamp01(0.42 + this.motionFallbackHold * 0.8),
          };
        }
      ...
      buildNodes();
      // Initial geometric connection fragments removed: the web now forms only through interaction.
      // connectNodes();
      buildArchiveVisuals();
